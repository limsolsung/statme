...
export default function AddTask({ onAdd }: AddTaskProps) {
  ...
  return (
    <form onSubmit={handleSubmit} className="border-t-2 border-border-dark p-3 bg-surface">
        ...
        {STAT_OPTIONS.map((s) => {
          ...
          return (
            <button
              ...
              className={`border-2 rounded px-2.5 py-1 transition-all active:translate-x-0.5 active:translate-y-0.5 active:shadow-none flex items-center gap-1 ${
                isActive
                  ? 'shadow-[2px_2px_0px]'
      ...
      <div className="flex gap-2">
        ...
        <button
          type="submit"
          disabled={!label.trim()}
          className="w-9 h-9 bg-accent border-2 border-accent-hover rounded shadow-[2px_2px_0px] shadow-accent-hover text-white text-lg flex items-center justify-center disabled:opacity-30 active:shadow-none active:translate-x-0.5 active:translate-y-0.5"
